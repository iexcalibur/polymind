...
const TestCustomNoteBlockSchema = defineBlockSchema({
  flavour: 'polymind:note-block-video',
  props: internal => ({
    text: internal.Text(),
  }),
  metadata: {
...
const TestInvalidNoteBlockSchema = defineBlockSchema({
  flavour: 'polymind:note-invalid-block-video',
  props: internal => ({
    text: internal.Text(),
  }),
  metadata: {
...
const TestRoleBlockSchema = defineBlockSchema({
  flavour: 'polymind:note-block-role-test',
  metadata: {
    version: 1,
    role: 'content',
    parent: ['polymind:note'],
...
const TestParagraphBlockSchema = defineBlockSchema({
  flavour: 'polymind:test-paragraph',
  metadata: {
    version: 1,
    role: 'test',
    parent: ['@content'],
...
describe('schema', () => {
  it('should be able to validate schema by role', () => {
    ...
    doc.addBlock('polymind:paragraph', {}, rootId);
    expect(consoleMock.mock.calls[0]).toSatisfy((call: unknown[]) => {
      return typeof call[0] === 'string';
    });
    expect(consoleMock.mock.calls[1]).toSatisfy((call: unknown[]) => {
    ...
    consoleMock.mockClear();
    doc.addBlock('polymind:note', {}, rootId);
    doc.addBlock('polymind:paragraph', {}, noteId);
    doc.addBlock('polymind:paragraph', {}, paragraphId);
  ...
  it('should glob match works', () => {
    ...
    doc.addBlock('polymind:note-block-video', {}, noteId);
    expect(consoleMock).not.toBeCalled();

    doc.addBlock('polymind:note-invalid-block-video', {}, noteId);
    expect(consoleMock.mock.calls[0]).toSatisfy((call: unknown[]) => {
      return typeof call[0] === 'string';
    });
    expect(consoleMock.mock.calls[1]).toSatisfy((call: unknown[]) => {
  ...
  it('should be able to validate schema by role', () => {
    ...
    const rootId = doc.addBlock('polymind:page', {});
    const noteId = doc.addBlock('polymind:note', {}, rootId);
    const roleId = doc.addBlock('polymind:note-block-role-test', {}, noteId);

    doc.addBlock('polymind:paragraph', {}, roleId);
    doc.addBlock('polymind:paragraph', {}, roleId);
    ...
    consoleMock.mockClear();
    doc.addBlock('polymind:test-paragraph', {}, roleId);
    doc.addBlock('polymind:test-paragraph', {}, roleId);
    expect(consoleMock).not.toBeCalled();

    expect(doc.getBlocksByFlavour('polymind:test-paragraph')).toHaveLength(2);
  });
});
