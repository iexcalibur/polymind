import { generateElementId, sortIndex } from '@blocksuite/polymind-block-surface';
import type { ConnectorElementModel } from '@blocksuite/polymind-model';
...
export const replaceIdMiddleware = (job: TemplateJob) => {
  ...
  const regenerateBlockId = (data: SlotBlockPayload['data']) => {
    ...
    if (blockJson.flavour === 'polymind:surface-ref') {
      assertType<
        SnapshotNode<{
          reference: string;
        }>
    ...
    if (blockJson.flavour === 'polymind:surface') {
    ...
    if (blockJson.flavour === 'polymind:frame') {
...
export const createStickerMiddleware = (
  ...
  return (job: TemplateJob) => {
    ...
    const changeInserPosition = (blockJson: BlockSnapshot) => {
      if (blockJson.flavour === 'polymind:image' && blockJson.props.xywh) {
        const bound = Bound.deserialize(blockJson.props['xywh'] as string);
    ...
    const generateIndexMap = () => {
      ...
      job.walk(block => {
        if (block.props.index) {
          if (block.flavour === 'polymind:frame') {
            frameList.push({
              id: block.id,
