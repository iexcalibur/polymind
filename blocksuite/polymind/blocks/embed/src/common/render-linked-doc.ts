import { getSurfaceBlock } from '@blocksuite/polymind-block-surface';
import {
  type DocMode,
  ListBlockModel,
  NoteBlockModel,
...
export function isEmptyNote(note: BlockModel) {
  return note.children.every(block => {
    return (
      block.flavour === 'polymind:paragraph' &&
...
export function getTitleFromSelectedModels(selectedModels: DraftModel[]) {
  ...
  const isParagraph = (
    ...
    model.flavour === 'polymind:paragraph';
...
export function createLinkedDocFromSlice(
  ...
  linkedDoc.load(() => {
    ...
    const noteId = linkedDoc.addBlock('polymind:note', {}, rootId);
