...
> = (ctx, next) => {
  ...
  const mergeToCode: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    if (flavour !== 'polymind:code') return;
  ...
  const appendDivider: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    const id = doc.addBlock('polymind:divider', {}, parent, index + 1);
    if (!nextSibling) {
      nextSiblingId = doc.addBlock('polymind:paragraph', {}, parent);
    }
  ...
  const [result, resultCtx] = std.command
    ...
    .try(chain => [
      chain.pipe((_, next) => {
        if (['polymind:code', 'polymind:divider'].includes(flavour)) {
          return next();
        }
        return false;
      }),
