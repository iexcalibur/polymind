import { appendParagraphCommand } from '@blocksuite/polymind-block-paragraph';
import {
  CodeBlockModel,
  ListBlockModel,
  NoteBlockModel,
...
export class PageRootBlockComponent extends BlockComponent<RootBlockModel> {
  ...
  private _createDefaultNoteBlock() {
    ...
    const noteId = store.addBlock('polymind:note', {}, store.root?.id);
  ...
  private _getDefaultNoteBlock() {
    return (
      this.store.root?.children.find(
        child => child.flavour === 'polymind:note'
  ...
  override connectedCallback() {
    ...
    this.bindHotKey({
      ...
      ArrowUp: () => {
        ...
        while (current && !model) {
          if (current.flavour === 'polymind:note') {
            model = current;
          } else {
        ...
        if (!prevNote || prevNote.flavour !== 'polymind:note') {
        ...
        const notes = this.store.getModelsByFlavour('polymind:note');
