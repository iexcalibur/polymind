...
import { NoteBlockSchema } from '@blocksuite/polymind-model';
import {
...
export class RootViewExtension extends ViewExtensionProvider {
  ...
  private readonly _setupPage = (context: ViewExtensionContext) => {
    ...
    if (context.scope === 'preview-page') {
      context.register(
        BlockViewExtension('polymind:page', literal`affine-preview-root`)
      );
      return;
    }
    context.register(
      BlockViewExtension('polymind:page', literal`affine-page-root`)
    );
  ...
  private readonly _setupEdgeless = (context: ViewExtensionContext) => {
    ...
    context.register([
      BlockViewExtension('polymind:page', literal`affine-edgeless-root`),
