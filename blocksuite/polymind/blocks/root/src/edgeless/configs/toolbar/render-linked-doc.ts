import { isFrameBlock } from '@blocksuite/polymind-block-frame';
import { getSurfaceBlock, isNoteBlock } from '@blocksuite/polymind-block-surface';
import {
...
import { getBlockProps } from '@blocksuite/polymind-shared/utils';
...
export function createLinkedDocFromNote(
  ...
  linkedDoc.load(() => {
    const rootId = linkedDoc.addBlock('polymind:page', {
      title: new Text(docTitle),
    });
    linkedDoc.addBlock('polymind:surface', {}, rootId);
    transformer
...
export function createLinkedDocFromEdgelessElements(
  ...
  linkedDoc.load(() => {
    ...
    const surfaceId = linkedDoc.addBlock('polymind:surface', {}, rootId);
