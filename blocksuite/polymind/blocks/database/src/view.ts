import type { MenuOptions } from '@blocksuite/polymind-components/context-menu';
import {
  type ViewExtensionContext,
  ViewExtensionProvider,
} from '@blocksuite/polymind-ext-loader';
import { DatabaseBlockModel } from '@blocksuite/polymind-model';
...
export class DatabaseViewExtension extends ViewExtensionProvider<DatabaseViewExtensionOptions> {
  ...
  override setup(
    ...
    super.setup(context);
    context.register([
      FlavourExtension('polymind:database'),
      BlockViewExtension('polymind:database', literal`affine-database`),
      SlashMenuConfigExtension('polymind:database', databaseSlashMenuConfig),
    ]);
    if (options) {
      context.register(
