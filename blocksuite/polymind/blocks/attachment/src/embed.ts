...
import { FileSizeLimitProvider } from '@blocksuite/polymind-shared/services';
import {
  readImageSize,
  transformModel,
  withTempBlobData,
...
export const AttachmentEmbedConfigIdentifier =
  createIdentifier<AttachmentEmbedConfig>(
    'PolymindAttachmentEmbedConfigIdentifier'
  );
...
const embedConfig: AttachmentEmbedConfig[] = [
  {
    ...
    check: model =>
      model.store.schema.flavourSchemaMap.has('polymind:image') &&
...
async function turnIntoImageBlock(model: AttachmentBlockModel) {
  if (!model.store.schema.flavourSchemaMap.has('polymind:image')) {
  ...
  transformModel(model, 'polymind:image', imageProp);
}
