import type { SurfaceBlockModel } from '@blocksuite/polymind-block-surface';
import { Overlay } from '@blocksuite/polymind-block-surface';
...
export class EdgelessFrameManager extends GfxExtension {
  ...
  private _addFrameBlock(bound: Bound) {
    ...
    const id = this.gfx.doc.addBlock('polymind:frame', props, surfaceModel);
...
export function isFrameBlock(element: unknown): element is FrameBlockModel {
  return !!element && (element as BlockModel).flavour === 'polymind:frame';
}
