...
export class RichText extends WithDisposable(ShadowlessElement) {
  ...
  private readonly _inlineEditor$ = signal<PolymindInlineEditor | null>(null);
  ...
  private _init() {
    ...
    // init inline editor
    this._inlineEditor$.value = new InlineEditor<PolymindTextAttributes>(
      this._yText,
      {
  ...
  accessor embedChecker: <
    TextAttributes extends PolymindTextAttributes = PolymindTextAttributes,
  >(
  ...
  @property({ attribute: false })
  accessor markdownMatches: InlineMarkdownMatch<PolymindTextAttributes>[] = [];
