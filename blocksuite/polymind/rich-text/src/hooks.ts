...
import { isStrictUrl } from '@blocksuite/polymind-shared/utils';
import type {
  BeforeinputHookCtx,
...
const autoIdentifyLink = (ctx: HookContext<PolymindTextAttributes>) => {
...
function handleExtendedAttributes(
  ctx:
    | BeforeinputHookCtx<PolymindTextAttributes>
    | CompositionEndHookCtx<PolymindTextAttributes>
) {
...
export const onVBeforeinput = (
  ctx: BeforeinputHookCtx<PolymindTextAttributes>
) => {
...
export const onVCompositionEnd = (
  ctx: CompositionEndHookCtx<PolymindTextAttributes>
) => {
  handleExtendedAttributes(ctx);
};
