...
import { SmallDocIcon } from '@blocksuite/polymind-components/icons';
import {
  FrameBlockModel,
  NoteBlockModel,
  NoteDisplayMode,
...
import { matchModels, stopPropagation } from '@blocksuite/polymind-shared/utils';
...
export class EdgelessAutoConnectWidget extends WidgetComponent<RootBlockModel> {
  ...
  private _initLabels() {
    ...
    const surfaceRefs = this.block.std.store
      .getBlocksByFlavour('polymind:surface-ref')
    ...
    this._disposables.add(
      this.store.slots.blockUpdated.subscribe(payload => {
        if (payload.flavour === 'polymind:surface-ref') {
          switch (payload.type) {
            case 'add':
