import { addSiblingAttachmentBlocks } from '@blocksuite/polymind-block-attachment';
import { insertDatabaseBlockCommand } from '@blocksuite/polymind-block-database';
import { insertEmptyEmbedIframeCommand } from '@blocksuite/polymind-block-embed';
import { insertImagesCommand } from '@blocksuite/polymind-block-image';
import { insertLatexBlockCommand } from '@blocksuite/polymind-block-latex';
import {
  canDedentListCommand,
  canIndentListCommand,
  dedentListCommand,
  indentListCommand,
} from '@blocksuite/polymind-block-list';
import { updateBlockType } from '@blocksuite/polymind-block-note';
import {
  canDedentParagraphCommand,
  canIndentParagraphCommand,
  dedentParagraphCommand,
  indentParagraphCommand,
} from '@blocksuite/polymind-block-paragraph';
import { DefaultTool, getSurfaceBlock } from '@blocksuite/polymind-block-surface';
import { insertSurfaceRefBlockCommand } from '@blocksuite/polymind-block-surface-ref';
import { insertTableBlockCommand } from '@blocksuite/polymind-block-table';
...
import { insertInlineLatex } from '@blocksuite/polymind-inline-latex';
import { toggleLink } from '@blocksuite/polymind-inline-link';
import {
  formatBlockCommand,
  formatNativeCommand,
  formatTextCommand,
...
import type { PolymindTextStyleAttributes } from '@blocksuite/polymind-shared/types';
import {
  createDefaultDoc,
...
import type { PolymindLinkedDocWidget } from '@blocksuite/polymind-widget-linked-doc';
...
const textToolActionItems: KeyboardToolbarActionItem[] = [
  {
    ...
    icon: TextIcon(),
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('polymind:paragraph'),
    action: ({ std }) => {
  ...
  {
    name: 'CodeBlock',
    showWhen: ({ std }) => std.store.schema.flavourSchemaMap.has('polymind:code'),
    icon: CodeBlockIcon(),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
  ...
  },
  {
    ...
    icon: DividerIcon(),
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('polymind:divider'),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
  ...
  },
  {
    ...
    showWhen: ({ std, rootComponent: { model } }) =>
      std.store.schema.flavourSchemaMap.has('polymind:table') &&
      !isInsideBlockByFlavour(std.store, model, 'polymind:edgeless-text'),
    action: ({ std }) => {
      std.command
        .chain()
  ...
  },
  {
    ...
    action: ({ rootComponent: { model }, std }) => {
      ...
      if (index === -1) return;
      const calloutId = store.addBlock('polymind:callout', {}, parent, index + 1);
      if (!calloutId) return;
      const paragraphId = store.addBlock('polymind:paragraph', {}, calloutId);
...
const listToolActionItems: KeyboardToolbarActionItem[] = [
  {
    ...
    showWhen: ({ std }) => std.store.schema.flavourSchemaMap.has('polymind:list'),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
  ...
  items: [
    {
      ...
      icon: NewPageIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('polymind:embed-linked-doc'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
    ...
    },
    {
      ...
      showWhen: ({ std, rootComponent }) => {
        ...
        return std.store.schema.flavourSchemaMap.has('polymind:embed-linked-doc');
      },
      action: ({ rootComponent, closeToolPanel }) => {
        ...
        assertType<PolymindLinkedDocWidget>(linkedDocWidget);
  ...
  items: [
    {
      ...
      icon: ImageIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('polymind:image'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
    ...
    },
    {
      ...
      icon: LinkIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('polymind:bookmark'),
      action: async ({ std }) => {
    ...
    },
    {
      ...
      icon: TeXIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('polymind:latex'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
  ...
  items: [
    {
      ...
      showWhen: ({ std }) => {
        return std.store.schema.flavourSchemaMap.has('polymind:embed-iframe');
      },
      action: async ({ std }) => {
        std.command
          .chain()
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('polymind:embed-youtube'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('polymind:embed-github'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('polymind:embed-figma'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('polymind:embed-loom'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
...
const documentGroupFrameToolGroup: DynamicKeyboardToolPanelGroup = ({
  ...
  const frameModels = store
    .getBlocksByFlavour('polymind:frame')
  ...
  items: [
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('polymind:database'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
        ...
        ...(
          ...
          action: ({ std }) => {
            const payload = {
              ...
              } satisfies PolymindTextStyleAttributes,
            };
            std.command
              .chain()
              .try(chain => [
...
export const defaultKeyboardToolbarConfig: KeyboardToolbarConfig = {
  items: [
    moreToolPanel,
    // TODO(@L-Sun): add ai function in PolyMind side
