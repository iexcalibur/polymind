...
export const sideMap = new Map([
  ...
  ['polymind:surface:frame', { top: 28 }],
  // includes group element
  ['polymind:surface:group', { top: 20 }],
  // has only one shape element
  ['polymind:surface:shape', { top: 26, bottom: -26 }],
]);
...
export function autoUpdatePosition(
  ...
  const isInline = flavour === 'polymind:note';
...
export function renderToolbar(
  ...
  const actions = [
    flavour,
    `custom:${flavour}`,
    hasSurfaceScope ? ['polymind:surface:*', 'custom:affine:surface:*'] : [],
    'polymind:*',
    'custom:affine:*',
  ]
    .flat()
  ...
  // Avoids shaking
  if (flavour === 'polymind:note' && context.std.range.value) {
