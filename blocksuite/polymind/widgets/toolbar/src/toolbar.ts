import { DatabaseSelection } from '@blocksuite/polymind-block-database';
import { EdgelessLegacySlotIdentifier } from '@blocksuite/polymind-block-surface';
import { TableSelection } from '@blocksuite/polymind-block-table';
import {
  darkToolbarStyles,
...
export class PolymindToolbarWidget extends WidgetComponent {
  ...
  updateWithSurface(
    ...
    if (activated && surface) {
      ...
      if (hasLocked) {
        flavour = 'polymind:surface:locked';
      } else {
        if (paired.length === 1) {
          flavour = paired[0][0];
          if (flavour === 'polymind:surface:shape' && paired[0][1].length === 1) {
      ...
      if (!sideOptions) {
        ...
        if (flavours.has('polymind:surface:frame')) {
          sideOptions = sideMap.get('polymind:surface:frame') ?? null;
        } else if (flavours.has('polymind:surface:group')) {
          sideOptions = sideMap.get('polymind:surface:group') ?? null;
        }
      }
    }
    ...
    if (IS_MOBILE) {
      ...
      const syncMobileTextSelection = () => {
        ...
        batch(() => {
          ...
          flavour$.value = 'polymind:note';
          placement$.value = toolbarRegistry.getModulePlacement('polymind:note');
      ...
      std.selection.filter$(BlockSelection).subscribe(selections => {
        ...
        batch(() => {
          ...
          placement$.value = toolbarRegistry.getModulePlacement(
            flavour,
            flavour === 'polymind:note' ? 'top' : 'top-start'
          );
