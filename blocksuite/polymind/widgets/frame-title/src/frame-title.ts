import { parseStringToRgba } from '@blocksuite/polymind-components/color-picker';
import {
  ColorScheme,
  FrameBlockModel,
  isTransparent,
...
export class PolymindFrameTitle extends SignalWatcher(
    ...
    _disposables.add(
      doc.slots.blockUpdated.subscribe(payload => {
        if (
          ...
          (payload.type === 'add' && payload.flavour === 'polymind:frame')
        ) {
          this._nestedFrame = this._isInsideFrame();
        }
