import { EdgelessFrameManagerIdentifier } from '@blocksuite/polymind-block-frame';
import {
  CanvasElementType,
...
import { FontFamilyIcon } from '@blocksuite/polymind-components/icons';
import {
  mountShapeTextEditor,
  SHAPE_OVERLAY_HEIGHT,
  SHAPE_OVERLAY_WIDTH,
  ShapeComponentConfig,
} from '@blocksuite/polymind-gfx-shape';
import {
  insertEdgelessTextCommand,
  mountTextElementEditor,
} from '@blocksuite/polymind-gfx-text';
import type {
  Connection,
...
export class EdgelessAutoCompletePanel extends WithDisposable(LitElement) {
  ...
  private _addFrame() {
    ...
    const props = this.std.get(EditPropsStore).applyLastProps('polymind:frame', {
      title: new Y.Text(`Frame ${frameIndex}`),
      xywh: serializeXYWH(...xywh),
      presentationIndex: frameMgr.generatePresentationIndex(),
    });
    const id = this.crud.addBlock('polymind:frame', props, surfaceBlockModel);
  ...
  private _addNote() {
    ...
    store.addBlock('polymind:paragraph', { type: 'text' }, id);
  ...
  private _showNoteOverlay() {
    ...
    const background = this.edgeless.std
      .get(ThemeProvider)
      .getColorValue(
        this.edgeless.std.get(EditPropsStore).lastProps$.value['polymind:note']
          .background,
