...
import { ViewportElementProvider } from '@blocksuite/polymind-shared/services';
import {
  autoScroll,
  getScrollContainer,
    ...
    const getAllNodeFromTree = (): BlockComponent[] => {
      ...
      this.host.view.walkThrough(node => {
        ...
        if (
          ...
          !PolymindPageDraggingAreaWidget.excludeFlavours.includes(
            view.model.flavour
          )
        ) {
          blocks.push(view);
...
declare global {
  interface HTMLElementTagNameMap {
    [AFFINE_PAGE_DRAGGING_AREA_WIDGET]: PolymindPageDraggingAreaWidget;
  }
}
