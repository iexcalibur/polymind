...
} from '@blocksuite/polymind-block-embed';
import { ParagraphBlockComponent } from '@blocksuite/polymind-block-paragraph';
import { DropIndicator } from '@blocksuite/polymind-components/drop-indicator';
import {
...
import { surfaceRefToEmbed } from '@blocksuite/polymind-shared/adapters';
import {
...
export class DragEventWatcher {
    ...
    if (edge === 'right' && matchModels(dropModel, [ListBlockModel])) {
      ...
      if (
        snapshot.content.every(block =>
          schema.safeValidate(block.flavour, 'polymind:list')
        )
      ) {
      ...
      const placement =
        isDropOnNoteBlock &&
        schema.safeValidate(snapshot.content[0].flavour, 'polymind:note')
          ? 'in'
          : edge === 'top'
            ? 'before'
            : 'after';
  ...
  private readonly _onPageDrop = (
    ...
    if (
      ...
      snapshot.content.every(block => block.flavour === 'polymind:note')
    ) {
    ...
    if (
      ...
      if (dragPayload.from?.docId === this.widget.store.id) {
        ...
        const walk = (block: BlockSnapshot) => {
          if (block.flavour === 'polymind:surface') {
            Object.values(
  ...
  private readonly _rewriteSnapshotXYWH = (
    ...
    const rewrite = (block: BlockSnapshot) => {
      if (block.flavour === 'polymind:surface') {
        ...
        if (block.flavour === 'polymind:embed-iframe') {
          ...
          block.flavour === 'polymind:attachment' ||
          block.flavour === 'polymind:bookmark' ||
          block.flavour.startsWith('polymind:embed-')
        ) {
        ...
        if (block.flavour === 'polymind:image') {
  ...
  private readonly _dropAsGfxBlock = (
    ...
    const groupByParent = groupBy(snapshot.content, block =>
      schema.safeValidate(block.flavour, 'polymind:surface')
        ? 'polymind:surface'
        : schema.safeValidate(block.flavour, 'polymind:page')
          ? 'polymind:page'
    ...
    if (!groupByParent.empty?.length) {
      ...
      if (groupByParent['polymind:surface']) {
        const content = groupByParent['polymind:surface'];
      ...
      if (groupByParent['polymind:page']) {
        const content = groupByParent['polymind:page'];
        const pageSnapshot = {
          ...snapshot,
          content,
        };
      ...
      const content = snapshot.content.filter(block =>
        schema.safeValidate(block.flavour, 'polymind:note')
      );
  ...
  private async _dropToModel(
    ...
    try {
      ...
      if (snapshot.content.length === 1) {
        const [first] = snapshot.content;
        if (first.flavour === 'polymind:embed-linked-doc') {
