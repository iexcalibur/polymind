...
import { PolymindLink } from '../polymind-link';
  ...
  actions: [
    {
      ...
      content(cx) {
        ...
        if (!(target instanceof PolymindLink)) return null;
      ...
      actions: [
        {
          ...
          run(ctx) {
            ...
            if (!(target instanceof PolymindLink)) return;
    ...
    },
    {
      ...
      actions: [
        ...
        },
        {
          ...
          when(ctx) {
            const target = ctx.message$.peek()?.element;
            if (!(target instanceof PolymindLink)) return false;
      ...
      when(ctx) {
        ...
        const bookmarkSchema = schema.flavourSchemaMap.get('polymind:bookmark');
