import { whenHover } from '@blocksuite/polymind-components/hover';
import { RefNodeSlotsProvider } from '@blocksuite/polymind-inline-reference';
import type { ReferenceInfo } from '@blocksuite/polymind-model';
import {
  ParseDocUrlProvider,
  ToolbarRegistryIdentifier,
} from '@blocksuite/polymind-shared/services';
import { affineTextStyles } from '@blocksuite/polymind-shared/styles';
import type { PolymindTextAttributes } from '@blocksuite/polymind-shared/types';
import { normalizeUrl } from '@blocksuite/polymind-shared/utils';
...
export class PolymindLink extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    polymind-link a:hover [data-v-text='true'] {
  ...
  // see https://github.com/toeverything/PolyMind/issues/1540
  ...
  get inlineEditor() {
    const inlineRoot = this.closest<InlineRootElement<PolymindTextAttributes>>(
      `[${INLINE_ROOT_ATTR}]`
    );
  ...
  override render() {
    const linkStyle = {
      color: 'var(--polymind-link-color)',
      fill: 'var(--polymind-link-color)',
  ...
  accessor delta: DeltaInsert<PolymindTextAttributes> = {
