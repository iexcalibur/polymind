import { LatexExtension } from '@blocksuite/polymind-inline-latex';
...
export const BoldItalicMarkdown = InlineMarkdownExtension<PolymindTextAttributes>(
  {
    name: 'bolditalic',
...
export const BoldMarkdown = InlineMarkdownExtension<PolymindTextAttributes>({
  name: 'bold',
...
export const ItalicExtension = InlineMarkdownExtension<PolymindTextAttributes>({
  name: 'italic',
...
export const StrikethroughExtension =
  InlineMarkdownExtension<PolymindTextAttributes>({
    name: 'strikethrough',
    pattern: /.*~{2}([^\s][^~]*[^\s])~{2}\s$|.*~{2}([^\s~])~{2}\s$/,
    action: ({
      inlineEditor,
...
export const CodeExtension = InlineMarkdownExtension<PolymindTextAttributes>({
  name: 'code',
  pattern: /.*`([^\s][^`]*[^\s])`\s$|.*`([^\s`])`\s$/,
  action: ({ inlineEditor, prefixText, inlineRange, pattern, undoManager }) => {
    const match = prefixText.match(pattern);
