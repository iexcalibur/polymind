...
import type {
  PolymindInlineEditor,
  PolymindTextAttributes,
...
function getCombinedFormatFromInlineEditors(
  inlineEditors: [PolymindInlineEditor, InlineRange | null][]
): PolymindTextAttributes {
  const formatArr: PolymindTextAttributes[] = [];
  ...
  return formatArr.reduce((acc, cur) => {
    const newFormat: PolymindTextAttributes = {};
    for (const key in acc) {
      const typedKey = key as keyof PolymindTextAttributes;
...
function getSelectedInlineEditors(
  ...
  filter: (
    inlineRoot: InlineRootElement<PolymindTextAttributes>
  ) => InlineEditor<PolymindTextAttributes> | []
) {
  return blocks.flatMap(el => {
    const inlineRoot = el.querySelector<
      InlineRootElement<PolymindTextAttributes>
    >(`[${INLINE_ROOT_ATTR}]`);
...
function handleCurrentSelection(
  ...
  handler: (
    ...
    inlineEditors: InlineEditor<PolymindTextAttributes>[]
  ) => { textAttributes: PolymindTextAttributes } | boolean | void
): Chain<InitCommandCtx & { textAttributes: PolymindTextAttributes }> {
  return chain.try(chain => [
    ...
    chain.pipe((ctx, next) => {
      const selectedInlineEditors = Array.from<InlineRootElement>(
        ...
        .map((el): PolymindInlineEditor => el.inlineEditor);
