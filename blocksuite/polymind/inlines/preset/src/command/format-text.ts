import { clearMarksOnDiscontinuousInput } from '@blocksuite/polymind-rich-text';
import { getSelectedBlocksCommand } from '@blocksuite/polymind-shared/commands';
import type {
  PolymindTextAttributes,
  PolymindTextStyleAttributes,
...
export const formatTextCommand: Command<{
  ...
  styles: PolymindTextStyleAttributes;
  mode?: 'replace' | 'merge';
}> = (ctx, next) => {
              ...
              Object.entries(styles).map(([key, value]) => {
                if (typeof value === 'boolean') {
                  return [
                    key,
                    (inlineEditor.marks &&
                      inlineEditor.marks[key as keyof PolymindTextAttributes]) ||
                    (delta &&
                      delta.attributes &&
                      delta.attributes[key as keyof PolymindTextAttributes])
                      ? null
                      : value,
                  ];
                }
