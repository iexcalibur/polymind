...
import { PolymindSchemas } from '../../schemas.js';
...
function normalizeDeltaForSnapshot(
  delta: DeltaInsert<PolymindTextAttributes>[],
...
function simplifyBlockForSnapshot(
  ...
  if (block.flavour === 'polymind:paragraph' || block.flavour === 'polymind:list') {
  ...
  if (block.flavour === 'polymind:callout') {
    simplified.emoji = block.props.emoji;
  }

  if (block.flavour === 'polymind:attachment') {
  ...
  const children = (block.children ?? [])
    .filter(child => child.flavour !== 'polymind:surface')
...
describe('snapshot to markdown', () => {
  ...
  test('imports frontmatter metadata into doc meta', async () => {
    const schema = new Schema().register(PolymindSchemas);
