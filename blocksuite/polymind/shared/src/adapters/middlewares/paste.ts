...
import type { PolymindTextAttributes } from '../../types';
...
class PasteTr {
  ...
  focusPasted = () => {
    ...
    const cursorBlock =
      this.pointState.model.flavour === 'polymind:code' || !this.lastSnapshot
  ...
  constructor(
    ...
    this.firstSnapshotIsPlainText =
      this.firstSnapshot.flavour === 'polymind:paragraph' &&
  ...
  merge() {
    ...
    if (this.pointState.model.flavour === 'polymind:code') {
      this._mergeCode();
      return;
    }
...
function flatNote(snapshot: SliceSnapshot) {
  if (snapshot.content[0]?.flavour === 'polymind:note') {
