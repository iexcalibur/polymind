  ...
  ({ slots, docCRUD, assetsManager }) => {
    ...
    const replaceSurfaceRefIdSubscription = afterImportBlock$
      ...
      .subscribe(model => {
        ...
        if (idMap.has(original)) {
          ...
          model.props.refFlavour === 'polymind:frame' &&
    ...
    const beforeImportBlockSubscription = slots.beforeImport
      ...
      .subscribe(payload => {
        ...
        if (snapshot.flavour === 'polymind:page') {
          const index = snapshot.children.findIndex(
            c => c.flavour === 'polymind:surface'
          );
          if (index !== -1) {
        ...
        // Should be re-paired.
        if (['polymind:attachment', 'polymind:image'].includes(snapshot.flavour)) {
        ...
        if (snapshot.flavour === 'polymind:surface') {
