import { FrameBlockModel } from '@blocksuite/polymind-model';
  ...
  const getPrev = (model: BlockModel) => {
    ...
    if (index > 0) {
      ...
      if (parent.role === 'root' && model.role === 'hub') {
        while (prev && prev.flavour !== 'polymind:note') {
 ...
 * ```
 *
 * NOTE: this method will skip the `polymind:note` block
 */
export function getNextContentBlock(
  ...
  while (currentBlock) {
    ...
    if (nextSibling) {
      // Assert nextSibling is not possible to be `polymind:page`
