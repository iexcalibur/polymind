...
export const synced: InitFn = (collection: Workspace, id: string) => {
  ...
  docSyncedPage.load(() => {
    ...
    const rootId = docSyncedPage.addBlock('polymind:page', {
      title: new Text('Synced - Page View'),
    });

    docSyncedPage.addBlock('polymind:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedPage.addBlock('polymind:note', {}, rootId);
  ...
  docSyncedEdgeless.load(() => {
    // Add root block and surface block at root level
    const rootId = docSyncedEdgeless.addBlock('polymind:page', {
      title: new Text('Synced - Edgeless View'),
    });

    docSyncedEdgeless.addBlock('polymind:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedEdgeless.addBlock('polymind:note', {}, rootId);
  ...
  docMain.load(() => {
    // Add root block and surface block at root level
    const rootId = docMain.addBlock('polymind:page', {
      title: new Text('Home doc, having synced blocks'),
    });

    const surfaceId = docMain.addBlock('polymind:surface', {}, rootId);
    const noteId = docMain.addBlock('polymind:note', {}, rootId);
