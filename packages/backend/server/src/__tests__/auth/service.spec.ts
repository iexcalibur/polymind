...
test('should be able to sign in by password', async t => {
  ...
  const signedInUser = await auth.signIn('u1@polymind.pro', '1');

  t.is(signedInUser.email, 'u1@polymind.pro');
});

test('should throw if user not found', async t => {
  const { auth } = t.context;

  await t.throwsAsync(() => auth.signIn('u2@polymind.pro', '1'), {
    message: 'Wrong user email or password: u2@polymind.pro',
  });
});
...
test('should throw if password not match', async t => {
  ...
  await t.throwsAsync(() => auth.signIn('u1@polymind.pro', '2'), {
    message: 'Wrong user email or password: u1@polymind.pro',
  });
});
...
test('should be able to change password', async t => {
  ...
  let signedInU1 = await auth.signIn('u1@polymind.pro', '1');
  t.is(signedInU1.email, u1.email);

  await auth.changePassword(u1.id, 'hello world affine');

  await t.throwsAsync(
    () => auth.signIn('u1@polymind.pro', '1' /* old password */),
    {
      message: 'Wrong user email or password: u1@polymind.pro',
    }
  );

  signedInU1 = await auth.signIn('u1@polymind.pro', 'hello world affine');
  t.is(signedInU1.email, u1.email);
});
...
test('should be able to change email', async t => {
  ...
  await auth.changeEmail(u1.id, 'u2@polymind.pro');

  await t.throwsAsync(() => auth.signIn('u1@polymind.pro' /* old email */, '1'), {
    message: 'Wrong user email or password: u1@polymind.pro',
  });

  signedInU1 = await auth.signIn('u2@polymind.pro', '1');
  t.is(signedInU1.email, 'u2@polymind.pro');
});
...
test('should be able to sign in different user in a same session', async t => {
  ...
  const u2 = await auth.signUp('u2@polymind.pro', '1');
