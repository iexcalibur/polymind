...
test('should set doc owner', async t => {
  ...
  const user = await models.user.create({ email: 'u1@polymind.pro' });
...
test('should transfer doc owner', async t => {
  const user = await models.user.create({ email: 'u1@polymind.pro' });
  const user2 = await models.user.create({ email: 'u2@polymind.pro' });
  const workspace = await create();
  const docId = 'fake-doc-id';
...
test('should count doc user roles', async t => {
  ...
  const users = await Promise.all([
    models.user.create({ email: 'u1@polymind.pro' }),
    models.user.create({ email: 'u2@polymind.pro' }),
  ]);
