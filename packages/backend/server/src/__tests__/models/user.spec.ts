...
test('should create a new user', async t => {
  ...
  t.is(user.email, 'test@polymind.pro');

  const user2 = await t.context.user.getUserByEmail('test@polymind.pro');

  t.not(user2, null);
  t.is(user2!.email, 'test@polymind.pro');
});
...
test('should update an user', async t => {
  ...
  const user2 = await t.context.user.update(user.id, {
    email: 'test2@polymind.pro',
  });

  t.is(user2.email, 'test2@polymind.pro');
});
...
test('should return null for non existing user', async t => {
  const user = await t.context.user.getUserByEmail('test@polymind.pro');
...
test('should fulfill user', async t => {
  ...
  const user2 = await t.context.user.fulfill('test2@polymind.pro');
...
test('should paginate users', async t => {
  ...
  await Promise.all(
    Array.from({ length: 100 }).map((_, i) =>
      t.context.user.create({
        ...
        email: `test-paginate-${i}@polymind.pro`,
  ...
  t.deepEqual(
    ...
    Array.from({ length: 10 }).map((_, i) => `test-paginate-${i}@polymind.pro`)
  );
});
