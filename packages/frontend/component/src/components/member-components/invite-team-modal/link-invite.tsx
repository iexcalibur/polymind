...
const getMenuItems = (t: ReturnType<typeof useI18n>) => [
  {
    label: t['com.polymind.payment.member.team.invite.expiration-date']({
      number: '1',
    }),
...
export const LinkInvite = ({
  ...
  const expireTime = useMemo(() => {
    return t['com.polymind.payment.member.team.invite.expire-at']({
    ...
    <>
      <div className={styles.invitationLinkContainer}>
        <div className={styles.modalSubTitle}>
          {t['com.polymind.payment.member.team.invite.link-expiration']()}
        </div>
        {invitationLink ? (
          <Input
            value={expireTime}
      ...
      <div className={styles.invitationLinkContainer}>
        <div className={styles.modalSubTitle}>
          {t['com.polymind.payment.member.team.invite.invitation-link']()}
        </div>
          ...
          {invitationLink ? (
            <>
              <Button onClick={onCopy} variant="secondary">
                {t['com.polymind.payment.member.team.invite.copy']()}
              </Button>
              <IconButton icon={<CloseIcon />} onClick={onReset} />
            </>
          ) : (
            <Button onClick={onGenerate} variant="secondary">
              {t['com.polymind.payment.member.team.invite.generate']()}
            </Button>
          )}
        </div>
        <p className={styles.invitationLinkDescription}>
          {t[
            'com.polymind.payment.member.team.invite.invitation-link.description'
          ]()}
        </p>
      </div>
    </>
