...
export interface PolymindCommandOptions {
  id: string;
...
export interface PolymindCommand {
  readonly id: string;
...
export function createPolymindCommand(
  options: PolymindCommandOptions
): PolymindCommand {
  return {
    id: options.id,
    run: options.run,
    ...
    category: options.category ?? 'polymind:general',
    get label() {
