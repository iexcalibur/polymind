...
export function setupSpaceAIEntry(panel: PolymindAIPanelWidget) {
  ...
  // In scenarios like (2), some browsers (see [ISSUE](https://github.com/toeverything/PolyMind/issues/11541))
    ...
    if (AIProvider.actions.chat && isSpaceEvent(event)) {
      ...
      if (selection && selection.isCollapsed() && selection.from.index === 0) {
        ...
        if (
          ...
          block.model.flavour !== 'polymind:paragraph'
        )
          return;
