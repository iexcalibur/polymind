...
export const useSnapshotPage = (
  ...
  const polymindWorkspace = useService(WorkspaceService).workspace;
  ...
  const page = useMemo(() => {
    ...
    const historyShellWorkspace = getOrCreateShellWorkspace(
      docCollection.id,
      workspacesService.getWorkspaceFlavourProvider(polymindWorkspace.meta)
    );
