...
export class PolyMindUpdateProvider extends Provider<GithubUpdateInfo> {
  static configFeed(options: UpdateProviderOptions): CustomPublishOptions {
    return {
      provider: 'custom',
      feedUrl: 'https://polymind.pro/api/worker/releases',
      updateProvider: PolyMindUpdateProvider,
      ...options,
    };
  }
...
type FileParts =
  | ['polymind', string, VersionDistribution, VersionPlatform, VersionArch]
  | [
      'polymind',
      string,
      `${'canary' | 'beta'}.${number}`,
...
export function availableForMyPlatformAndInstaller(
  ...
  //  in form of:
  //   polymind-${build}-${buildSuffix}-${distribution}-${platform}-${arch}.${installer}
