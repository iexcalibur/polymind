...
import { getPagesCount } from '@polymind-test/kit/utils/filter';
...
} from '@polymind-test/kit/utils/workspace';
...
test.skip('create multi workspace in the workspace list', async ({
  ...
  await page.evaluate(async () => {
    await new Promise(resolve => {
      window.addEventListener('polymind:workspace:change', resolve, {
        once: true,
      });
    });
  });
