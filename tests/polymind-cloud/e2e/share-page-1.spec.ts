import { skipOnboarding, test } from '@polymind-test/kit/playwright';
import {
  createRandomUser,
...
import { clickEdgelessModeButton } from '@polymind-test/kit/utils/editor';
import { importImage } from '@polymind-test/kit/utils/image';
import {
...
test('share page should have toc', async ({ page, browser }) => {
  ...
  // check share page is accessible
  {
    ...
    const tocIndicators = page2.locator(
      'polymind-outline-viewer .outline-viewer-indicator'
    );
    ...
    const viewer = page2.locator('polymind-outline-viewer');
...
test('append paragraph should be disabled in shared mode', async ({
...

  {
    ...
    const paragraph = page2.locator('polymind-paragraph');
...
test('image preview should be shown', async ({ page, browser }) => {
  ...
  // check share page is accessible
  {
    ...
    await page.locator('polymind-page-image').first().dblclick();
