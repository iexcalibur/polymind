import { copyByKeyboard } from '@polymind-test/kit/utils/keyboard';
...
test.describe('AIBasic/Chat', () => {
  ...
  test(`should send message and receive AI response:
    ...
    await utils.chatPanel.makeChat(
      page,
      'Introduce PolyMind to me. Answer in 500 words.'
    );
    ...
    if (!(await page.getByTestId('ai-loading').isVisible())) {
      ...
      await utils.chatPanel.waitForHistory(page, [
        {
          role: 'user',
          content: 'Introduce PolyMind to me. Answer in 500 words.',
        },
        {
          role: 'assistant',
          status: 'loading',
  ...
  test('should support stop generating', async ({
    ...
    await utils.chatPanel.makeChat(
      page,
      'Introduce PolyMind to me. Answer in 5000 words.'
    );
    ...
    await utils.chatPanel.waitForHistory(page, [
      {
        role: 'user',
        content: 'Introduce PolyMind to me. Answer in 5000 words.',
      },
      {
        role: 'assistant',
        status: 'transmitting',
  ...
  test('chat with ask ai input in page mode', async ({
    ...
    await utils.editor.askAIWithText(
      page,
      'PolyMind is an open source all in one workspace.'
    );
