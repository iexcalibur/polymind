...
test('undo/redo should work correctly after clipping', async ({ page }) => {
  ...
  await expect(page.locator('polymind-edgeless-note')).toHaveCount(1);
  ...
  await button.click();
  await expect(page.locator('polymind-edgeless-note')).toHaveCount(2);
...
test('continuous undo and redo (note block add operation) should work', async ({
  ...
  let count = await countBlock(page, 'polymind-edgeless-note');
  expect(count).toBe(1);
  ...
  count = await countBlock(page, 'polymind-edgeless-note');
  expect(count).toBe(4);
