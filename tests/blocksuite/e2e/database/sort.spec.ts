...
test('database sort with multiple rules', async ({ page }) => {
  ...
  await page.locator('polymind-menu').getByText('Name').click();
  await waitNextFrame(page);

  // Add second sort rule: Age ascending
  await page.getByText('Add sort').click();
  await page.locator('polymind-menu').getByText('Age').click();
  await waitNextFrame(page);

  // Get all rows after sorting
  const rows = await page.locator('polymind-database-row').all();
  ...
  const rowsAfterDesc = await page.locator('polymind-database-row').all();
  ...
  const rowsAfterRemove = await page.locator('polymind-database-row').all();
