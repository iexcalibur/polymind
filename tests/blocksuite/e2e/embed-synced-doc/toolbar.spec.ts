...
test.describe('embed-synced-doc toolbar', () => {
  ...
  test('can change embed synced doc to card view', async ({ page }) => {
    ...
    const syncedDoc = page.locator('polymind-embed-synced-doc-block');
    ...
    const toolbar = page.locator(
      ...
      'polymind-page-root > div > affine-toolbar-widget editor-toolbar'
    );
    await expect(toolbar).toBeVisible();
    ...
    const embedSyncedBlock = page.locator('polymind-embed-linked-doc-block');
    expect(await embedSyncedBlock.count()).toBe(1);
  });
});
