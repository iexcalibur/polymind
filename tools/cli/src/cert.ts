import { type Path, ProjectRoot } from '@polymind-tools/utils/path';
...
export class CertCommand extends Command {
  ...
  domain = Option.String('--domain', {
    description:
      'Generate certificates for given domain. e.g. "polymind.localhost"',
  });
  ...
  private createCert(domain: string) {
    if (!this.checkInstalled(CA_PEM_PATH)) {
      this.logger.error(
        'CA not installed. Please run `yarn polymind cert --install` first.'
      );
      process.exit(1);
    }
    ...
    this.exec(
      `openssl req -new -key ${keyPath} -out ${csrPath} -config ${confPath} -subj "/C=/ST=/O=/localityName=/commonName=${domain}/organizationalUnitName=/emailAddress=${domain}@polymind.pro/"`
    );
    this.exec(
  ...
  private installCa() {
    ...
    this.exec(
      `openssl req -new -newkey rsa:2048 -days 1024 -nodes -x509 -subj "/C=/ST=/O=${CA_ORG}/localityName=/commonName=${CA_NAME}/organizationalUnitName=Developers/emailAddress=dev@polymind.pro/" -keyout ${CA_KEY_PATH} -out ${CA_PEM_PATH}`
    );
    this.trustCa(CA_PEM_PATH);
  }

  private trustCa(pem: Path) {
    this.logger.info(`Trusting PolyMind Dev Self Signed CA`);
    this.exec(
