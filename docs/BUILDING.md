...
PolyMind client has both **Node.js** & **Rust** toolchains.
...
```sh
git clone https://github.com/toeverything/PolyMind
```
...
Note: use `strip` from system instead of `binutils` if you are running MacOS.

```sh
yarn polymind @polymind/native build
```
...
```shell
# there are `polymind-local`, `polymind-migration`, `polymind-local`, `polymind-prototype` e2e tests,
#   which are run under different situations.
yarn workspace @polymind-test/polymind-local e2e
```
