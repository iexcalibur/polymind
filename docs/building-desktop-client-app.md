# Building PolyMind Desktop Client App
...
Before you start building PolyMind Desktop Client Application, please following the same steps in [BUILDING#Prerequisites](./BUILDING.md#prerequisites) to install Node.js and Rust.
...
> repos/PolyMind/.github/workflows/release-desktop.yml contains real order to build the desktop client app, but here we will explain the steps in a more detailed way. Up-to date.
...
```shell
BUILD_TYPE=canary yarn polymind @polymind/electron build

BUILD_TYPE=canary yarn polymind @polymind/electron generate-assets
```
...
```shell
BUILD_TYPE=canary SKIP_WEB_BUILD=1 HOIST_NODE_MODULES=1 yarn polymind @polymind/electron make
```
...
yarn polymind @polymind/electron package
yarn polymind @polymind/electron make-squirrel
yarn polymind @polymind/electron make-nsis
```
...
Finished 2 bundles at:
  › Artifacts available at: <polymind-repo>/packages/frontend/apps/electron/out/canary/make
```
